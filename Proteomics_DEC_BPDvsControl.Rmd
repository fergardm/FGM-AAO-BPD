...
samples <- samples %>% mutate(cond_type=case_when(jensen_condition=="control" & type=="NF" ~ "NPA Control",
                                       ...
                                       jensen_condition=="fallecido" & type=="NF" ~ "NPA BPD",
                                       jensen_condition=="fallecido" & type=="TQ" ~ "TA BPD"),
...
colors.mixo <- factor(conditions, levels=c("NPA BPD", "NPA Control"), 
                      labels = c("#FFC20A", "#0C7BDC"))
summary(conditions)
...
plsda <- plsda(protein_matrix, conditions, ncomp = 4) # Restrict to 2 components 
...
tune.splsda.protein$choice.keepX # Optimal variables for each components
...
ROC.dim3 <- roc(DiscriminantScoresTable.dim3$conditions, 
                DiscriminantScoresTable.dim3$`NPA BPD`) 
...
colnames(experimental.design) <- c("Control","BPD")
linear.fit <- lmFit(protein_log_filter, experimental.design)
contrast.matrix <- makeContrasts(BPD-Control,
...
protein.names <- c(protein.names, "Unknown_1")

id.names <- read_excel("idmapping_2024_07_25.xlsx", col_names = TRUE)
id.names <- id.names %>% mutate(`protein name` = gsub("\\s.*", "", id.names$`Gene Names`))

clusters.str <- read_excel("./STRING/clusters_DEC_BPD_NPA.xlsx")
clusters.str <- as.data.frame(clusters.str)
clusters.str$`protein name`[5] <- "GBA1"
...
outside.proteins <- c("TFF3_HUMAN", "MESD_HUMAN", "H2B1K_HUMAN", "HDGR2_HUMAN",
                      "GFPT1_HUMAN")
outside.protein.genes <- c("TFF3", "MESD", "H2BC12", "HDGFL2", "GFPT1")
...
roc(samples.roc$class, glm.fit.panel.sel.5$fitted.values, plot=TRUE,
    legacy.axes=TRUE,print.auc=TRUE,print.auc.x=.65,print.auc.y=0.23, grid=TRUE,
...
plot.roc(samples.roc$class, glm.fit.panel.sel.20$fitted.values, col="#004D40", add=TRUE,
         print.auc=TRUE,
         print.auc.x=.65,print.auc.y=0.53, lwd=4, ci=TRUE)
par(pty="m")
...
annotations <- read.table(file = "./STRING/string_protein_annotations (1).tsv",
                          header = F, sep= "\t")
...
annotation.merged.table <- annotation.merged.table %>%
  mutate(cluster.summary=case_when(cluster.number=="1" ~ "Protein translation regulation",
                                   ...
                                   cluster.number=="3" ~ "Reactive oxygen species regulation",
                                   cluster.number=="4" ~ "Secreted proteins",
...
```{r}
selection <- c("KRT2","DPYSL2","H2BC12","SGSH","HMOX1","KHSRP","VNN2")
...
heatmap.2(as.matrix(heat.prots), 
          col = colfunc(256),
          scale = "row",
          trace = "none",
          dendrogram="both",
          ...
          keysize = 1.5, 
          key.title = NA,
          ColSideColors = c(rep("#FFC20A",57),rep("#0C7BDC",74)),
          labRow = rownames(heat.prots),
          cexRow = 0.4,
          srtRow = 0.5,
          key.xlab = "Z-score",
          key.ylab = "",
...
variables <- variables %>%
  mutate(Condition=case_when(jensen_condition=="bpd" ~ "BPD",
                             ...
                             jensen_condition=="fallecido" ~ "BPD"),
...
new.variables %>%
  ...
  as_gt() %>%
  gt::tab_source_note(gt::md("*IVM: Invasive Mechanical Ventilation; NIV: Non-Invasive Ventilation; nCPAP: nasal CPAP; HFNC: High Flow Nasal Cannula ; LFNG: Low Flow Nasal Cannula*")) %>%
  gt::gtsave(filename = "Table_1_def_decbpd_control.docx")
