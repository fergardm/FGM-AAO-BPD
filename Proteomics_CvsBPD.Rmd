...
#### Hospital PCA (no deceased)

```{r}

hospitals <- samples$hospital_abrev[samples$cond_type!="NPA Deceased"]

plotIndiv(pca, comp = c(1, 2), ind.names = F, 
          group = hospitals,  
          legend = TRUE, title = 'PCA comp 1 - 2')

svglite::svglite("Figuras adicionales CHEST/PCA_hospitals.svg")
plotIndiv(pca, comp = c(1, 2), ind.names = F, 
          group = hospitals, 
          ellipse = T, 
          ellipse.level = 0.95,
          legend = TRUE, title = 'PCA comp 1 - 2')
dev.off()
pdf("Figuras adicionales CHEST/PCA_hospitals.pdf", width = 7, height = 6)
plotIndiv(pca, comp = c(1, 2), ind.names = F, 
          group = hospitals,
          ellipse = T, 
          ellipse.level = 0.95,
...
#### Hospital PCA (deceased)

```{r}

hospitals.dec <- samples$hospital_abrev

protein_matrix_dec <- t(protein_log_filter)
protein_matrix_dec <- scale(protein_matrix_dec, center = FALSE, scale = TRUE)

pca.dec <- pca(protein_matrix_dec,ncomp = 2, center = TRUE, scale = TRUE,
           max.iter = 100)

plotIndiv(pca.dec, comp = c(1, 2), ind.names = F, 
          group = hospitals.dec,  
          legend = TRUE, title = 'PCA comp 1 - 2')

svglite::svglite("Figuras adicionales CHEST/PCA_hospitals_dec.svg")
plotIndiv(pca.dec, comp = c(1, 2), ind.names = F, 
          group = hospitals.dec, 
          ellipse = T, 
          ellipse.level = 0.95,
          legend = TRUE, title = 'PCA comp 1 - 2')
dev.off()
pdf("Figuras adicionales CHEST/PCA_hospitals_dec.pdf", width = 7, height = 6)
plotIndiv(pca.dec, comp = c(1, 2), ind.names = F, 
          group = hospitals.dec,
          ellipse = T, 
          ellipse.level = 0.95,
...
train.splsda.protein <- splsda(X.train, Y.train, ncomp = 3, keepX = c(10,300)) 
...
MVA.cmv(protein_matrix,conditions, model = "PLS-DA",progress = TRUE,
         ncomp = 2,crit.inn="NMC")
MVA.test(protein_matrix,conditions, model = "PLS-DA", nperm = 1000, progress = TRUE,
         ncomp = 2,p.method = "fdr", cmv = TRUE)
```
